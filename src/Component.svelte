<script lang="ts">
    ...
    function handleChange(e) {
        if (selectedItem) {
            ...
            if (selEvent) {
                selEvent({selectedObj:selectedItem});
            }
        }
    ...
    $: if (formApi && field) {
        ...
        unsubscribe = formField?.subscribe(async (fieldValue) => {
            ...
            if (value && !initialItemsPromise) {
                ...
                const item = (await initialItemsPromise).find(
                    (item) => item[valueFieldName] === value,
                );
    ...
    $: if (
        ...
        (dataSourceType === "budibase" && searchEvent)
    ) {
        const parsedLoading =
            loading?.toLowerCase() === "true" || loading === "1";
        ...
        if (parsedLoading && !loadingResolver) {
            ...
            console.log('these are the real results')
            console.log(results)
            if (!results) {
                results = "[]";
            }
